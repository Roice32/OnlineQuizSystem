...
import QuizResultsDisplay from "../../Components/ResultsAndStatistics/QuizResultDisplay";
            ...
            <div>
              {quizHistory.quizResultHeaders.map((header, index) => (
                <div key={index} className="mb-2 p-5 rounded-[50px] border-2 border-gray-500">
                  ...
                  <button className="p-2 block w-72 mx-auto bg-teal-700 text-white rounded-full text-center leading-12 text-lg no-underline mt-4" onClick={() => getQuizResult(header.userId, header.quizId)}>View more details</button>
                </div>
              ))}
            </div>
          )
