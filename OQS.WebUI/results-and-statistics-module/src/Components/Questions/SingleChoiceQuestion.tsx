...
export default function SingleChoiceQuestionDisplay({
  question,
  activeQuizId,
  selectedAnswer,
  ...
  const handleChange = (e) => {
    const singleChoiceAnswer =
      selectedAnswer?.singleChoiceAnswer === e.target.value
        ? undefined
        : e.target.value;
    const answer: Answer = {
              ...
              {question.choices?.map((choice, i) => (
                <div key={i} className="flex items-center justify-center mb-2">
                  <input
                    ...
                    name={question.id}
                    value={choice}
                    checked={selectedAnswer?.singleChoiceAnswer === choice}
                  ...
                  <label
                    ...
                    className={`cursor-pointer p-2 rounded-full border border-gray-300 w-full max-w-md text-left ${
                      selectedAnswer?.singleChoiceAnswer === choice
