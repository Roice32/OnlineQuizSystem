...
namespace QQS.CoreWebAPI.Tests
{
    public class JoinLiveQuizTests : IClassFixture<WebApplicationFactory<ApplicationContextForTesting>>
    {
        ...
        //     }
        [Fact]
        public async Task JoinRoom_ValidConnection_AddsUserToGroup()
        {
            // Arrange
            var contextMock = new Mock<ApplicationDBContext>();
            var handlerMock = new Mock<Handler>();
            var hubContextMock = new Mock<IHubCallerClients>();

            var hub = new LiveQuizzesHub(contextMock.Object, handlerMock.Object);
            hub.Clients = hubContextMock.Object;

            var conn = new ConnectionRequest()
                { UserId = Guid.Parse("5b048913-5df0-429f-a42b-051904672e4d"),
                    Code = "xxAwXD" 
                };

            // Act
            await hub.JoinRoom(conn);

            // Assert
            contextMock.Verify(c => c.LiveQuizzes.FindAsync(conn.Code), Times.Once);
            contextMock.Verify(c => c.SaveChangesAsync(), Times.Once);
            hubContextMock.Verify(h => h.Group("myQuizCode").SendAsync("ReceiveMessage", It.IsAny<string>()),
                Times.Once);
        }
    }
}